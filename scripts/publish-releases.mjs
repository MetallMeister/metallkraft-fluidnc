...
assert.ok(tags.length && tags.every(t=>/^v\d+\.\d+\.\d+$/.test(t)),'Pass version tags');
...
for(const tag of tags){
  ...
  if(!release)release=await api('/releases','POST',{tag_name:tag,name:`MetallKraft UI ${tag}`,body,draft:true,prerelease:false});
  ...
  release=await api(`/releases/${release.id}`,'PATCH',{body,draft:false,make_latest:tag===tags.at(-1)?'true':'false'});
